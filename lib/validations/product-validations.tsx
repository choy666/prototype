...
import type { AttributeRequirementResult } from '@/lib/mercado-envios/attribute-requirements';
...
export const getValidations = (
  ...
  attributeEvaluation?: AttributeRequirementResult | null
): ValidationItem[] => {
  const requiredAttributes = recommendedAttributes.filter((attr) => attr.required);
  const fallbackHasRequiredAttributes =
  ...
  const hasRequiredAttributes = attributeEvaluation
    ? attributeEvaluation.missingRequired.length === 0 &&
      attributeEvaluation.invalidValues.length === 0
    : fallbackHasRequiredAttributes;

  const hasRequiredAttributeRules = attributeEvaluation
    ? attributeEvaluation.totals.required > 0
    : requiredAttributes.length > 0;
  ...
  return [
    ...
    },
    {
      ...
      isRequired: hasRequiredAttributeRules,
      icon: <Tag className='h-4 w-4' />,
    },
    {
      field: 'height',
